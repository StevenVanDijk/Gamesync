...
/**
 * URL of the backend GOG proxy.
 * Default: '/api/gog' (relative, works on Vercel and any same-origin host).
 */
export const GOG_BACKEND_URL = new InjectionToken<string>('GOG_BACKEND_URL', {
  providedIn: 'root',
  factory: () => '/api/gog',
});
...
export class GogApiService {
  ...
  private readonly backendUrl = inject(GOG_BACKEND_URL);
  private readonly connectionSvc = inject(StoreConnectionService);
  private readonly logger = inject(LoggingService);

  /** Returns the GOG authorization URL to redirect the user to. */
  ...
  /**
   * Fetch owned games for a GOG connection, refreshing the token first if expired.
   */
  getOwnedGames(config: GogConnectionConfig, connectionId: string, storeId: string): Observable<Game[]> {
   ...
   */
  private withFreshToken(config: GogConnectionConfig, connectionId: string): Observable<string> {
